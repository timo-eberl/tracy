...
// if VS Code says it can't find emscripten, you need to add its path to
// includePath something like
...
// typical kitchen: assume 4x3 m (12 m^2) floor, 2.4 m height, target
// illuminance is ~200 lux this means a total of 2400 lumen must reach the floor
// assume the light is 2.4 m centered above the floor, the solid angle to the
// floor is ~1.38 sr
// -> the luminous intensity of the point light should be 1739 lm/sr
// in reality we might have a fluorescent tube or multiple individual smaller
// LEDs for this, which typically are not isotropic (10.1.5), instead their
// intensity is higher downwards however, since we use an isotropic point light,
// our total luminous flux must be higher to achieve the same luminance
// downwards. typical fluorescent tube has 21.5 W. luminous flux: (1739 lm/sr) *
// (4pi sr) = ~21850 lm convert to radiant flux (172 lm/W for a fluorescent
// tube): ~127 W

// PointLight should be removed once indirect lighting is implemented.
// Emissive light: visible surface is probably around 1 m^2, so 21.5 W flux
// correspond to 21.5 W/m^2 radiosity

// room dimensions: (3,2.4,4)

// position, radiant flux (W), color
PointLight simple_light = {
    {0, 2.38, 0}, 127, {1, 1, 1}}; // only for render_fast
Sphere scene[] = {
    // center, radius, color, type
...
Vec vec_hadamard_prod(Vec a, Vec b) {
...
double *summed_weights_buffer =
    NULL; // stores the summed weights of the samples
int buffer_width = 0;
int buffer_height = 0;
...
bool intersect_sphere(Ray r, Sphere s, HitInfo *hit) {
  ...
  if (discriminant < 0) {
    ...
    hit->t = t0 > 0
                 ? t0
                 : t1; // if the first intersection is behind ray, use the other
...
// for tone mapping we convert our color values to a single luminance value to
// combat the problem of washing out our image described in 4.2.5
...
void initialize_buffers() {
  ...
  if (image_buffer == NULL || summed_weighted_radiance_buffer == NULL ||
      summed_weights_buffer == NULL || width != buffer_width ||
...
bool is_in_shadow(Vec surf_pos, Vec surf_normal, Vec light_pos) {
  ...
  shadow_ray.origin =
      vec_add(shadow_ray.origin, vec_scale(surf_normal, SELF_OCCLUSION_DELTA));
  HitInfo shadow_ray_hit;
  bool is_in_shadow =
      intersect_scene(shadow_ray, &shadow_ray_hit, &(Sphere *){NULL});
  if (is_in_shadow) { // check if occluder is further away than light source
    double hit_dist_sq = shadow_ray_hit.t * shadow_ray_hit.t;
    double light_dist_sq =
        vec_length_squared(vec_sub(light_pos, shadow_ray.origin));
...
Vec refract(Vec incident, Vec normal, double eta, bool *total_int_refl) {
  ...
  *total_int_refl = k < 0;
  if (*total_int_refl) { // total internal reflection
    ...
    Vec refl_dir = vec_add(vec_scale(incident, eta),
                           vec_scale(normal, eta * cos_i - sqrt(k)));
...
Vec radiance_from_ray_simple(Ray r) {
  ...
  Vec normal =
      hit.inside ? vec_scale(hit.n, -1.0) : hit.n; // if inside, flip normal
  ...
  double irradiance =
      (cos_theta * simple_light.radiant_flux) / (dist_sq * 4 * M_PI); // 10.1.5
  ...
  Vec sphere_color =
      hit_sphere->type == DIFFUSE ? hit_sphere->color : (Vec){1, 1, 1};
...
double random_double() { return (double)rand() / (RAND_MAX + 1.0); }
...
Vec sample_uniform_hemisphere(Vec normal) {
  ...
  // local -> world
  Vec sample_world = vec_add(
      vec_add(vec_scale(u, sample_local.x), vec_scale(v, sample_local.y)),
      vec_scale(w, sample_local.z));

  // ensure the sample is in the correct hemisphere
  if (vec_dot(sample_world, normal) < 0.0) {
    return vec_scale(sample_world, -1.0);
  }
  return sample_world;
}
...
Vec radiance_from_ray(Ray r, int depth) {
  ...
  case DIFFUSE: {
    ...
    refl_ray.origin =
        vec_add(refl_ray.origin, vec_scale(normal, SELF_OCCLUSION_DELTA));
    ...
    Vec radiance = vec_scale(
        vec_hadamard_prod(lambertian_brdf, incoming_radiance), cos_theta);
    // we now calculated the expected value, but because we integrate over the
    // hemisphere we still need to multiply with 2 pi (8.3.3)
  ...
  case MIRROR: {
    ...
    // instead we describe perfect reflection as L_r = L_i * rho, where rho is
    // just a ratio
    Ray refl_ray = {hit.p, reflect(r.dir, normal)};
    refl_ray.origin =
        vec_add(refl_ray.origin, vec_scale(normal, SELF_OCCLUSION_DELTA));
    Vec radiance = vec_hadamard_prod(radiance_from_ray(refl_ray, depth + 1),
                                     hit_sphere->color);
    return radiance;
  }
  case REFRACTIVE: {
    ...
    if (reflectance > random_double()) { // do either reflection or refraction
      ...
      refr_ray.origin =
          vec_add(refr_ray.origin, vec_scale(normal, -SELF_OCCLUSION_DELTA));
      Vec refraction_radiance = radiance_from_ray(refr_ray, depth + 1);
      return refraction_radiance;
    }
  }
  default:
    assert(false); // material type not implemented
  }
}
...
void write_image_tone_mapped() {
  ...
  for (int y = 0; y < height; ++y)
    for (int x = 0; x < width; ++x) {
      ...
      // If this were a continuous integral, the sum of weights would be 1.0 and
      // this step unnecessary. But since we are doing a discrete sum, our total
      // weight will not be exactly 1.0, so we manually keep track of it.
...
void render_init(int p_width, int p_height, double p_cam_angle_x,
                 double p_cam_angle_y, double p_cam_dist, double p_focus_x,
  ...
  // Calculate Camera Position using spherical coordinates around the focus
  // point
  double cam_x =
      focus_point.x + p_cam_dist * sin(p_cam_angle_y) * cos(p_cam_angle_x);
  double cam_y = focus_point.y + p_cam_dist * sin(p_cam_angle_x);
  double cam_z =
      focus_point.z + p_cam_dist * cos(p_cam_angle_y) * cos(p_cam_angle_x);
...
uint8_t *render_refine(unsigned int n_samples) {
  ...
  // This factor scales our sample offsets to cover the desired range of the
  // filter. For sigma=0.5, radius=3 samples will range from -1.5 to 1.5
  const double sample_range_scale =
      GAUSS_SIGMA * GAUSS_FILTER_RADIUS_IN_SIGMA * 2.0;
  ...
  for (size_t sample_index = 0; sample_index < n_samples; ++sample_index) {
    ...
    // and then add them to the radiance buffer multiple times at different
    // pixels. also at the pixels at the edge will have less samples
    ...
    for (int y = 0; y < height; ++y)
      for (int x = 0; x < width; ++x) {
        ...
        double weight =
            gaussian_weight_2d(sample_offset_x, sample_offset_y, GAUSS_SIGMA);
