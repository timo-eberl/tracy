...
def generate_convergence_plot(log_path, output_path):
        ...
        for line in f:
            ...
            if line.startswith("VARIANT:") or line.startswith("VERSION:"):
                ...
                try:
                    ...
                    prev_time = data[current_variant]["time"][-1] if data[current_variant]["time"] else 0
    ...
    for i, (variant, results) in enumerate(sorted(data.items())):
        if not results["time"]: continue
        plt.plot(results["time"], results["rmse"], 
                 label=variant.upper(), 
                 color=get_color(i), marker='o', markersize=4, linewidth=2)
...
def generate_trend_plot(csv_path, output_path, max_versions=20):
    ...
    for i, mode in enumerate(sorted(data.keys())):
        ...
        if not plot_values: continue

        plt.plot(plot_indices, plot_values, label=mode.upper(), 
                 color=get_color(i), marker='s', markersize=5, linewidth=2)

    plt.xticks(range(len(window_versions)), window_versions, rotation=45)
    plt.ylim(bottom=0)
    plt.title(f"Historical Performance Trend (Last {len(window_versions)} builds)", fontsize=12)
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python generate_plots.py <log_path> <csv_path> <out_dir> [num_versions]")
        sys.exit(1)
    ...
    generate_trend_plot(csv_in, os.path.join(out_dir, "history_trend.png"), max_versions=num_v)
    
    print(f"Success: Plots generated for all detected modes in {out_dir}")