...
=======
            if ts > final_time:
                # Forward fill the line after completion
...
=======
    ...
    all_resampled_vals = [
        val
        for sublist in [resampled[m] for m in resampled]
        for val in sublist
        if val is not None
    ]
    y_limit = max(all_h_vals or [1.0]) * 1.1  # Using historical max as a baseline scale
